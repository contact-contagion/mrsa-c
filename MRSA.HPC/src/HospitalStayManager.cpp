...
HospitalStayManager::HospitalStayManager(unsigned int y1_length, unsigned int y2_length,
	...
	stays.push_back(std::pair<unsigned int, unsigned int>
				((unsigned int)repast::Random::instance()->createUniIntGenerator(1, 365 - y2_length).next(), y2_length));
	stays.push_back(std::pair<unsigned int, unsigned int>
				((unsigned int)repast::Random::instance()->createUniIntGenerator(1, 365 - y3_length).next(), y3_length));
	stays.push_back(std::pair<unsigned int, unsigned int>
